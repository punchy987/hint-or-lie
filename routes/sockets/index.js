...
const { makePersistence } =
  require(path.join(__dirname, 'persistence.js'));
  ...
  io.on('connection',(socket)=>{
    ...
    socket.on('getLeaderboard', async ()=>{
      try{
        ...
        socket.emit('leaderboardData', top.map(x=>({ deviceId:x.deviceId, pseudo:x.lastPseudo||'Joueur', wins:Number(x.wins||0) })));
    ...
    socket.on('getMyStats', async ({ deviceId })=>{
      ...
      socket.emit('myStats', doc ? { rp:Number(doc.rp||0), rounds:Number(doc.rounds||0), wins:Number(doc.wins||0), winsCrew:Number(doc.winsCrew||0), winsImpostor:Number(doc.winsImpostor||0) } : { rp:0, rounds:0, wins:0, winsCrew:0, winsImpostor:0 });
    });
    ...
    socket.on('joinRoom', ({ code, name, deviceId })=>{
      ...
      if (r.state === 'lobby' && r.timer?.phase === 'lobby'){
        clearRoomTimer(r); r.lobbyReady = new Set();
    ...
    socket.on('leaveRoom', ()=>{
      ...
      if (r.hostId === socket.id){ const first = r.players.keys().next().value; if (first) r.hostId = first; else rooms.delete(code); }
      socket.leave(code); joined.code = null; broadcast(io, code); socket.emit('leftRoom');
    });
    ...
    socket.on('playerReadyLobby', ({ ready })=>{
      ...
      if (r.lobbyReady.size === r.players.size && r.players.size >= 3){
        ...
        clearRoomTimer(r); io.to(joined.code).emit('lobbyCountdownCancelled');
      }
    });

    // Démarrer (hôte)
    socket.on('startRound', ()=>{
      const r = rooms.get(joined.code); if(!r) return; if (r.hostId !== socket.id) return socket.emit('errorMsg','Seul l\'hôte peut démarrer');
      controller.startRound(joined.code); socket.emit('actionAck', { action:'startRound', status:'ok' });
    });
    ...
    socket.on('submitVote', ({targetId})=>{
      ...
      if(!r.players.has(targetId)) return; const p = r.players.get(socket.id); if(!p) return;
      p.vote = targetId; socket.emit('voteAck');
    ...
    socket.on('resetScores', ()=>{
      const r = rooms.get(joined.code); if(!r) return; if (r.hostId !== socket.id) return socket.emit('errorMsg','Seul l\'hôte peut réinitialiser');
      for (const p of r.players.values()) p.score = 0; r.round = 0; r.state='lobby'; r.used={}; r.lobbyReady = new Set();
      clearRoomTimer(r); io.to(joined.code).emit('lobbyCountdownCancelled'); io.to(joined.code).emit('scoresReset'); broadcast(io, joined.code);
    });

    // Déconnexion
    socket.on('disconnect',()=>{
      const code = joined.code; if(!code) return; const r = rooms.get(code); if(!r) return;
      r.players.delete(socket.id);
      if (r.lobbyReady?.has(socket.id)){
        r.lobbyReady.delete(socket.id);
        io.to(code).emit('lobbyReadyProgress', { ready: r.lobbyReady.size, total: r.players.size });
