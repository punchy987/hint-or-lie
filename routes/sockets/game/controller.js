...
function createController({ io, upsertRoundResult, HINT_SECONDS, VOTE_SECONDS }){
  ...
  function startRound(code){
    ...
    for (const [id, p] of r.players.entries()){
      ...
      io.to(id).emit('roundInfo', {
        ...
        round: r.round,                 // ⬅️ envoie le n° de manche
      });
    }

    // progression phase
    io.to(code).emit('phaseProgress', {
      phase: 'hints', submitted: 0, total: ids.length, round: r.round   // ⬅️ round ici aussi
    });
  ...
  function maybeStartVoting(code){
    ...
    io.to(code).emit('allHints', {
      hints, domain: r.words?.domain || null, round: r.round           // ⬅️ round
    });

    io.to(code).emit('phaseProgress', {
      phase: 'voting', submitted: 0, total: r.players.size, round: r.round
    });

    startPhaseTimer(io, code, VOTE_SECONDS, 'voting', () => {
      const room = rooms.get(code); if (!room) return;
      // auto-vote sur soi si pas voté
  ...
  function finishVoting(code){
    ...
    // winners de manche (pour stats locales / firestore)
    ...
    io.to(code).emit('roundResult', {
      round: r.round,                                   // ⬅️ round dans le payload
      impostorId: impId,
    ...
    if (maxScore >= 10){
      ...
      io.to(code).emit('gameOver', { winners: winnersArr, round: r.round, autoReset: true }); // ⬅️ round + autoReset
